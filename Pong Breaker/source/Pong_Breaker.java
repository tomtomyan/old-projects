...
public class Pong_Breaker extends PApplet {
...
public void draw() {
  ...
  if (player1Lives == 0 || player2Lives == 0) {
    ...
    for (int i = 0; i < balls.size(); i++) {
      ...
      if ((balls.get(i)).getY() <= 50) {
        if (((balls.get(i)).getX()) < (p1x + 80) && ((balls.get(i)).getX()) > (p1x)) {
          ...
          float offset = - (p1x + 40 - balls.get(i).ballx) / 7 + 0.1f;
      ...
      if ((balls.get(i)).getY() >= 450) {
        if (((balls.get(i)).getX()) < (p2x + 80) && ((balls.get(i)).getX()) > (p2x)) {
          ...
          float offset = - (p2x + 40 - balls.get(i).ballx) / 7 - 0.1f;
    ...
    for (int i = 0; i < bricks.size(); i++) { //iterates for every brick
      ...
      float y1 = bricks.get(i).y + 12.5f; //adds the half height of a brick to the y value to get y value for center of the brick
      ...
      for (int j = 0; j < balls.size(); j++) { //iterates through every ball
        ...
        if (!bricks.get(i).cleared) { //if the brick hasn't been cleared yet
          if (x2 < x1 + 45 && x2 > x1 - 45) { //if the x value for the ball is within the center of the brick
            if (y2 < y1 + 17.5f && y2 > y1 - 17.5f) { //if the y value for the ball is within the center of the brick
              bricks.get(i).cleared = true; //clears the brick
              if (Math.min(Math.abs(y2-(y1+17.5f)), Math.abs(y2-(y1-17.5f))) < Math.min(Math.abs(x2-(x1-45)), Math.abs(x2-(x1+45)))) { //determines which side (top/bottom or left/right)
  ...
  if (val != null) {
    ...
    if (firstContact == false) {
      ...
      if (val.substring(0, 1).equals("1")) {
        p1x = (int) (Integer.parseInt(val.substring(1)) / 2.44f);
      } else if (val.substring(0, 1).equals("2")) {
        p2x = (int) (Integer.parseInt(val.substring(1)) / 2.44f);
...
public class Brick {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Pong_Breaker" };
    if (passedArgs != null) {
